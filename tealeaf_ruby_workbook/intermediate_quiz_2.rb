...
total_years = 0
munster_men = munsters.select {|name, info| info["age"] if info["gender"] == "male" }
munster_men.each do |name, info|
  total_years += info["age"]
end
puts total_years
...
#  I'm unclear on what we want to do here.

#5. A quick glance at the docs for the Ruby String class will have you scratching your head over the absence of any "word iterator" methods.
...
puts "test a is This.".split(/\W/).reverse.join(' ') + '.'
...
#the family's data got ransacked! The method, mess_with_demographics, alters varibles that exist outside of the method.
#therefore changes are made to the hashes that belong to each munster.
